...
public class CustomerNewGui {
	...
	private static CustomerNewGui newCustomerGuiInstance;

	/**
	 * Launch the application.
	 */
	public static CustomerNewGui getCustomerGuiInstance() {
		if (newCustomerGuiInstance == null) {
			newCustomerGuiInstance = new CustomerNewGui(new ContactDetailsValidator());
		}
		newCustomerGuiInstance.frame.setVisible(true);
		return newCustomerGuiInstance;
	}

	/**
	 * Create the application.
	 */
	private CustomerNewGui(ContactDetailsValidator customerInfoValidator) {
	...
	private void customerAndAccount() {
		ContactDetails contactDetails = createContactDetails();
		if (contactDetails.Validate(customerInfoValidator, new ArrayList<String>())) {
			// create customer && account
			ArrayList<Account> mainaccount = new ArrayList<Account>();
			mainaccount.add(new Account());
			Customer customer = new Customer(mainaccount);
			customer.setNationality(nationality.getSelectedItem().toString());
			customer.setGender(gender.getSelectedItem().toString());
			customer.setFullName(fullName.getText());
			customer.setUserName(createUniqueUserName(customer.getFullName(), contactDetails.getDateOfBirth()));
			customer.setPassword(String.valueOf(passwordField.getPassword()));
			customer.setContactDetails(contactDetails);
			customer.setNationalID(NationalId.getText());

			CustomerNewAccountGui window = new CustomerNewAccountGui(customer.getAccounts().get(0), customer,
					new AccountBalanceValidator());
			window.frame.setVisible(true);
			clearFields();
		} else {
			// show error message
			JOptionPane.showMessageDialog(frame, customerInfoValidator.brokenRules(contactDetails));
		}
	}
