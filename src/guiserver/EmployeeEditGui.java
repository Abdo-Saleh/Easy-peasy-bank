...
public class EmployeeEditGui {
	 ...
	 */
	public EmployeeEditGui(Employee employee, ContactDetailsValidator employeeInfoValidator) {
	...
	private void initialize() {
		...
		fullName.setText(this.employee.getFullName());
		...
		NationalId.setText(this.employee.getNationalID());
		...
		nationality.setSelectedItem(this.employee.getContactDetails().getAddress().getCountry());
		...
		dateOfBirth.setText(this.employee.getContactDetails().getDateOfBirth());
		...
		email.setText(this.employee.getContactDetails().getEmail());
		panel_1.add(email);

		phoneNum = new JTextField();
		phoneNum.setColumns(10);
		phoneNum.setBounds(485, 129, 193, 38);
		phoneNum.setText(this.employee.getContactDetails().getPhoneNumber());
		...
		street.setText(this.employee.getContactDetails().getAddress().getStreet());
		...
		postalCode.setText(this.employee.getContactDetails().getAddress().getPostalCode());
	...
	private void updateEmployee() {
		...
		if (updatedcontactDetails.Validate(employeeInfoValidator, new ArrayList<String>())) {
			// update customer
			this.employee.setFullName(fullName.getText());
			this.employee.setGender(gender.getSelectedItem().toString());
			this.employee.setNationality(nationality.getSelectedItem().toString());
			this.employee.setPassword(String.valueOf(passwordField.getPassword()));
			this.employee.setContactDetails(updatedcontactDetails);
			this.employee.setNationalID(NationalId.getText());
			Database.updateEmployee(this.employee);
			JOptionPane.showMessageDialog(frame,
			...
			// show error message
			JOptionPane.showMessageDialog(frame, employeeInfoValidator.brokenRules(updatedcontactDetails));
		}
	...
	private ContactDetails updateContactDetails() {
		// create address
		ContactDetails.Address address = this.employee.getContactDetails().getAddress();
		address.setStreet(street.getText());
		address.setPostalCode(postalCode.getText());
		address.setCountry(nationality.getSelectedItem().toString());
		// create contact details
		ContactDetails updatedcontactDetails = this.employee.getContactDetails();
